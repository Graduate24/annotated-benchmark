...
def load_flowdroid_results():
    ...
    try:
        with open(FLOWDROID_RESULTS_PATH, 'r', encoding='utf-8') as f:
            ...
            for rule_data in data:
                ...
                for result in results:
                    ...
                    flowdroid_item = {
                        ...
                        'class_name': class_name or sink_point.get('javaClass', ''),
            ...
            print(f"已加载 FlowDroid 结果数据")
            for cwe in CWES_TO_EVALUATE:
                print(f"  CWE-{cwe}: {len(flowdroid_data.get(cwe, []))} 个漏洞")
    except Exception as e:
...
def is_matching_in_call_chain(flowdroid_item, truth_item):
    ...
    for point in flowdroid_item.get('path', []):
        ...
        point_info = {
            ...
            'function': function,
            'class_name': class_name or point.get('javaClass', ''),
            'method_name': method_name,
...
def save_detailed_results(detailed_results):
    ...
    try:
        with open('flowdroid_detailed_results.json', 'w', encoding='utf-8') as f:
            json.dump(simplified_results, f, indent=2, ensure_ascii=False)
        print(f"详细评估结果已保存到 flowdroid_detailed_results.json")
    except Exception as e:
...
def save_evaluation_results(metrics):
    """保存评估结果到JSON文件"""
    try:
        with open('flowdroid_evaluation_results.json', 'w', encoding='utf-8') as f:
            json.dump(metrics, f, indent=2, ensure_ascii=False)
        print(f"评估结果已保存到 flowdroid_evaluation_results.json")
    except Exception as e:
        print(f"保存评估结果时出错: {e}")
